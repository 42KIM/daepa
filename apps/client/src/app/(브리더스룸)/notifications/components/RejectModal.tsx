    ...
    <Dialog open={isOpen} onOpenChange={close}>
      <DialogContent>
        ...
        <div className="relative flex flex-col gap-2">
          <p className="text-sm text-gray-800 dark:text-gray-400">요청을 거절하시겠습니까?</p>
          <textarea
            className={`min-h-[160px] w-full rounded-xl bg-gray-100 p-4 text-left text-[14px] focus:outline-none focus:ring-0 dark:bg-gray-600/50 dark:text-white`}
            value={rejectReason ?? ""}
            maxLength={300}
            onChange={(e) => {
              e.stopPropagation();
